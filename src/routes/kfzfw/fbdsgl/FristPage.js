...
@connect(({ appPublish, loading }) => {
    
    return {
        appPublish,
        loading: loading.effects['appPublish/querylist'],
    }
  })
class FristPage extends  Component{
    ...
    queryClick(){  //查询按钮 回调
        ...
        let {querystring,etime,stime,state} = this.state;
        this.props.dispatch({
            type:'appPublish/querylist',
    ...
    onChangeDispatherData(current,pageSize){  //翻页数据请求
        let {stime,etime,querystring,state} = this.state;
        this.props.dispatch({
            type:'appPublish/querylist',  
            querystring:querystring,
            state:state,
    ...
    onshowTotal(){   //分页器文字显示
        let {total,current} = this.props.appPublish.Pagination;
    ...
    render(){
        return (
         ...
         <Card title='待检应用管理'    bordered={false}  >
       <AppRelease
         pagination = {this.state.pagination}
         onInputChange={this.onInputChange.bind(this)} 
         queryClick={this.queryClick.bind(this)}
         titelcardssPaginationSet = {this.props.appPublish.Pagination}
